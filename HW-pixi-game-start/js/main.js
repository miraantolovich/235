...
function createLabelsAndButtons() {
    ...
    startButton.on("pointerout", e => e.target.alpha = 1.0); //concise arrow function
...
function gameLoop() {
    ...
    let amt = 6 * dt; //at 60 fps, will move around 10% of distance
    //lerp x and y values
    let newX = lerp(ship.x, mousePosition.x, amt);
    let newY = lerp(ship.y, mousePosition.y, amt);
    //keep ship on screen with clamp()
    let w2 = ship.width/2;
